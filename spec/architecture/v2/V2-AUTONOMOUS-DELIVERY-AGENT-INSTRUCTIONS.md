...
The helper is intentionally guarded: it confirms the PR is open, targets `main`, and still has the expected exact head SHA; it de-duplicates an already-emitted Work Order/head pair. The orchestrator must not use a chat message, local-only state, or CI status as the review notification substitute.
...
Only after the GitHub comment succeeds may the orchestrator mark the review entry `AWAITING_ARCHITECT_REVIEW`. If GitHub emission fails, keep the packet in a non-awaiting state and retry/escalate; never claim that the Architect was notified.
