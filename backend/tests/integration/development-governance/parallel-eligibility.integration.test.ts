...
describe('WORK-052 — parallel eligibility, conflicts, and assurance selection', () => {
  ...
  it('W052-AC03 — the real program state: the historical WORK-046/WORK-052 shared-surface coordination is DURABLE HISTORY (both merged — no active conflict; the pairwise view still reports the shared surface + the mutual coordination)', () => {
    ...
    // in-flight conflict partner: WORK-069 (Progressive Release & Runtime
    // Validation — the feedback binding layer, activated 2026-08-31 by the
    // architect's implementation instruction on branch
    // feat/WORK-069-progressive-release, grown from 26e2ada — the 2026-08-31
    // dogfooding-evidence merge) shares the static-architecture suite
    // surface with WORK-046 and WORK-052 — the SAME durable-history surface
    // pattern WORK-064/065/066/067/071/074 each shared while in flight
    // (WORK-067 was the prior live partner, merged as bde33cc via PR #103
    // and finalized by the WORK-067 post-merge finalization — PR #105;
    ...
    // WORK-071 merged 8604c8a5 via PR #96; and WORK-074 merged cdedd0ca via
    // PR #99 — all merged items are durable history). The frontier is the
    // authoritative live view: the ONE in-flight item on THIS branch's
    // canonical view is WORK-069 (it declares the static-architecture
    // suite in sharedIntegrationSurfaces, so the surface flag discipline
    // holds for the live pair; the parallel WORK-068 is an open PR from
    // the same base — declared in dependency-state knownConflicts — not an
    // in-flight record in THIS canonical view).
    expect(a046.conflictsWith.map((c) => c.workOrderId)).toEqual(['WORK-069']);
    expect(a052.conflictsWith.map((c) => c.workOrderId)).toEqual(['WORK-069']);
  ...
  it('W052-AC03 — the REAL frontier: 60 recorded items complete (WORK-074/WORK-071/WORK-065/WORK-066/WORK-067 all COMPLETE + FINALIZED since their merges); WORK-069 is the ONE in-flight record (the canonical view of this branch); nothing is blocked', () => {
    const frontier = realService.getFrontier();
    ...
    // complete — and then WORK-069 (Progressive Release & Runtime
    // Validation — the feedback binding layer) was ACTIVATED by the
    // architect on 2026-08-31 (the implementation instruction; every hard
    // edge verified complete on main): the ONE in-flight record on THIS
    // branch's canonical view (grown from 26e2ada; the parallel WORK-068 is
    // an open PR from the same base, declared in dependency-state
    // knownConflicts — not an in-flight record in THIS canonical view).
    // Nothing is blocked
    // (WORK-053..061 and WORK-070/072/073 are future-generation items not
    // recorded in program-state; WORK-068 likewise is not recorded in THIS
    // canonical view — its activation lives on its own branch).
    expect(frontier.inFlight.map((w) => w.id)).toEqual(['WORK-069']);
