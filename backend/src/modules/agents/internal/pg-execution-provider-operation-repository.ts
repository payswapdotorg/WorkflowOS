 ...
 * provider-operation ledger — PostgreSQL persistence for the keyed external
 * dispatch boundary (`wfos_execution_provider_operations` — migrations 0048 +
 * 0049).
 *
 * THE ROUND-9 CORRECTION. The round-8 review confirmed the durable ledger
 * (round 8) is real and correctly persisted, but found its TAKEOVER PROTOCOL
 * does not satisfy the stronger exactly-once contract — three blocking
 * issues, all in this file's state machine:
 *
 *   1. takeover re-executed the operation body (takeOver + drive again);
 *   2. complete()/fail() were NOT fenced by generation — a stale generation
 *      could resolve the row after a take-over (a stale FAIL could DEFEAT the
 *      recovery generation; a stale SUCCESS could win merely by racing);
 *   3. register() re-armed terminally FAILED rows — one key could resolve to
 *      FAILED/result-A then COMPLETED/result-B, so the key did not identify
 *      one immutable operation.
 *
 * The corrected design (the review's freeze — the ledger behaves like the
 * existing handoff lease):
 *
 *     operation key
 *        ↓
 *     one durable operation row          (idempotency_key PRIMARY KEY)
 *        ↓
 ...
 *        ↓                               (provider_operation_handle — 0049)
 *     one terminal result
 *
 ...
 *     FRESH key OPENS the operation (state 'pending', generation 1). An
 *     EXISTING row is returned as-is for convergence: 'completed' → REPLAY
 *     the stored submission; 'failed' → the stored terminal failure; 'pending'
 *     → await/take over. THE KEY IS IMMUTABLE: COMPLETED and FAILED are BOTH
 *     terminal — register NEVER re-arms a failed row (round 9 removed the
 *     round-8 re-arm: a true idempotency key identifies the LOGICAL OPERATION
 *     INVOCATION — one key, one operation, one terminal result; retryability
 *     is the driver mechanics' concern, never a second operation under the
 *     same key).
 *
 *   attachOperation(key, generation, handle): the OPERATION-IDENTITY CAS —
 *     durably records the provider-side identity of the ONE operation on the
 *     row ('pending' @ THIS generation, handle not yet recorded). The drive
 *     attaches the handle BEFORE the operation body runs (see the provider),
 *     so a recorded handle PROVES the operation was started, and an absent
 *     handle PROVES the body never ran.
 *
 *   complete(key, generation, submission): the GENERATION-FENCED resolution
 *     CAS — 'pending' @ THIS generation → 'completed' (+ the stored result).
 *     Only the ACTIVE driver's generation can resolve the operation: a stale
 *     generation (superseded by a take-over) is STRUCTURALLY INCAPABLE of
 *     resolving it (0 rows, both for a stale SUCCESS and for a stale FAILURE).
 *
 *   fail(key, generation, error): the symmetric generation-fenced CAS
 *     ('pending' @ THIS generation → 'failed', error stored). A CAS loss is a
 *     benign no-op: the row's recorded resolution stands (the loser re-reads
 *     the row and converges to the winner's outcome).
 *
 *   takeOver(key): the process-loss recovery CAS — 'pending' → 'pending' @
 *     generation + 1, RETURNING the NEW GENERATION TOKEN (the fencing token
 *     the recovering driver must present to complete/fail). The state stays
 *     pending (the outcome is still unresolved). Rejects a row that resolved
...
export interface ProviderOperationRecord {
   ...
   * migration 0049): recorded by attachOperation BEFORE the operation body
   * runs. Present ⇒ the operation was STARTED (a recovery driver resolves it
   * BY IDENTITY — never a body re-run); absent ⇒ the body never started.
   */
...
export interface ExecutionProviderOperationStore {
  ...
  /**
   * Durably record the PROVIDER OPERATION IDENTITY (the durable provider-side
   * identity of the ONE operation — migration 0049) on the row, BEFORE the
   * operation body runs. The CAS requires the row to be 'pending' at the
   ...
   * RESOLUTION CAS ('pending' @ `generation` → 'completed', the submission
   * stored as the operation's ONE terminal result). Returns
   * `completed === false` when the CAS lost — the caller's generation is
   * STALE (superseded by a take-over) or the row already resolved — and
   * `current` then holds the winner's row for convergence: a stale
   * generation is STRUCTURALLY INCAPABLE of resolving the operation, for a
   ...
   * CAS ('pending' @ `generation` → 'failed', the error stored). A CAS loss
   * (the generation is stale, or another driver resolved the row) is a benign
   * no-op: the row's recorded resolution stands; the stale driver's failure
   * is structurally DISCARDED (it can never defeat the active generation).
   */
  fail(idempotencyKey: string, generation: number, error: string): Promise<void>;

  /**
   * PR #46 round 9 (the process-loss recovery): TAKE OVER the drive of a
   * PENDING row — the recovery drive of the SAME row (never a second row):
   * the generation counter is incremented and the NEW GENERATION TOKEN is
   ...
   * of resolving the operation). The state stays 'pending' (the outcome is
   * still unresolved). The CAS rejects a row that resolved while the
  ...
  implements ExecutionProviderOperationStore
{
  ...
  async attachOperation(
    ...
    // The OPERATION-IDENTITY CAS: durably records the provider-side identity
    // of the ONE operation on the row — 'pending' @ the CALLER's generation,
    // with the handle not yet recorded. The drive attaches the handle BEFORE
    // the operation body runs, so a recorded handle PROVES the operation was
    // started (a recovery driver resolves it BY IDENTITY — never a body
    // re-run), and an absent handle PROVES the body never started. The FIRST
    // recorded handle is immutable for the operation (a later recovery
    // driver re-derives the SAME deterministic identity and simply uses the
    // recorded one).
  ...
  async complete(
    ...
    // The GENERATION-FENCED RESOLUTION CAS ('pending' @ THIS generation →
    // 'completed'). Only the ACTIVE driver's generation can resolve the
    // operation: concurrent drivers (an original owner + a take-over after
    // process loss) and a LATE stale driver's completion all funnel through
    // this single CAS — the losers affect 0 rows and replay the winner's
    // stored submission. PR #46 round 9: the `AND generation = $2` fence is
    // the fix for the round-8 stale-owner race (a stale generation could
    // previously resolve the row merely because it raced the recovery
    // generation to the CAS — for a success AND, worse, for a FAILURE that
    // defeated the recovery driver's later success).
    const result = await this.db.query<{ idempotency_key: string }>(
      `UPDATE wfos_execution_provider_operations
          SET state = 'completed',
  ...
  async fail(
    ...
    // The symmetric GENERATION-FENCED CAS ('pending' @ THIS generation →
    // 'failed'). A CAS loss is a benign no-op — the row's recorded
    // resolution stands; a STALE generation's failure is structurally
    // DISCARDED (it can never defeat the active recovery generation: the
    // round-8 review's blocking interleaving "takeover → stale FAIL → new
    // generation SUCCESS" is impossible by construction).
    await this.db.query(
      `UPDATE wfos_execution_provider_operations
          SET state = 'failed',
              error_message = $3,
  ...
  async takeOver(
    ...
    // from this CAS onward. The state stays 'pending' (the outcome is still
    // unresolved); the CAS serializes concurrent take-overs and rejects a
    // row that resolved while the caller's await window elapsed (the caller
    // converges instead of driving).
