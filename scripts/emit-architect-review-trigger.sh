...
The command emits one durable READY_FOR_ARCHITECT_REVIEW packet to the GitHub
PR conversation, then prints the exact user-facing Architect trigger.
EOF
  exit 2
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --work-order)
      [[ $# -ge 2 ]] || usage
      work_order="$2"; shift 2 ;;
    --head)
      [[ $# -ge 2 ]] || usage
      expected_head="$2"; shift 2 ;;
    --main)
      [[ $# -ge 2 ]] || usage
      expected_main="$2"; shift 2 ;;
    --verification)
      [[ $# -ge 2 ]] || usage
...

{
  ...
  printf 'ARCHITECT REVIEW: WorkflowOS PR #%s, Work Order %s, head %s\n' "$pr" "$work_order" "$expected_head"
} > "${TMPDIR:-/tmp}/workflowos-architect-review-$pr.txt"

comment_file="${TMPDIR:-/tmp}/workflowos-architect-review-$pr.txt"
