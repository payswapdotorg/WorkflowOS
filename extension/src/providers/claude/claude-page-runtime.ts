 ...
 *        → BLOCKED "Claude UI changed…" when none
 ...
 *     5. record conversation URL (/chat/<uuid>) as externalSessionRef (safe
 *        metadata only)
 ...
 *        ambiguous confirmations → BLOCKED "Claude requires user
...
export type { ClaudePageSession } from './claude-types.js';
import type {
  ClaudeAttachResult,
  ClaudeObservation,
  ClaudePageSession,
...
async function emitPhase(phase: ClaudePhase, detail?: string): Promise<void> {
...
async function emitObservation(observation: Omit<ClaudeObservation, 'executionId'>): Promise<void> {
  if (!state || state.stopped) return;
  const full: ClaudeObservation = { ...observation, executionId: state.session.executionId };
  const type =
...
export async function attach(session: ClaudePageSession): Promise<ClaudeAttachResult> {
  ...
  if ((login.element && !composerMatch.element) || LOGIN_PATH_PATTERN.test(location.pathname)) {
    await emitObservation({
      kind: 'blocked',
      reason: 'Please sign in to Claude.',
      output: 'Claude session is not authenticated.',
    });
  ...
  //     REQUIRE the coding-agent surface (Claude Code). A conversational Claude
  ...
  if (session.taskKind === 'implementation' && surface.surface !== 'coding-agent') {
    await emitObservation({
      kind: 'blocked',
      reason: 'Claude coding environment unavailable or unverified.',
      output:
        surface.surface === 'unknown'
          ? 'Could not confidently identify a Claude surface (Chat / Projects / Claude Code).'
          : `Detected surface "${surface.surface}" is not the coding-agent environment (Claude Code); conversational fallback is not permitted for implementation tasks.`,
    });
  ...
  if (session.taskKind === 'conversational' && surface.surface !== 'conversational-chat') {
    await emitObservation({
      kind: 'blocked',
      reason: 'Claude conversational surface unavailable or unverified.',
  ...
  if (!composer) {
    await emitObservation({
      kind: 'blocked',
      reason: 'Claude UI changed; automatic execution paused.',
  ...
  if (!session.promptSubmitted && !state.injectedExecutionIds.has(session.executionId)) {
    ...
    if (!sent) {
      await emitObservation({
        kind: 'blocked',
        reason: 'Claude send control unavailable.',
    ...
    await emitObservation({
      ...
      output: 'Prompt submitted to Claude.',
    });
  } else {
  ...
  // 5. Record the conversation URL once Claude transitions to /chat/<uuid>.
...
/**
 * Detect the current Claude surface (WORK-031). URL patterns are the
 * primary (HIGH) signal; page anchors confirm (MEDIUM). The Claude Code web
 * surface lives at /code (official Anthropic docs); Claude conversations
 * at /chat/<uuid> + the new-chat surface; Projects at /projects.
 */
export function detectSurface(): DetectedSurface {
  const path = location.pathname;
  if (CLAUDE_CODE_URL_PATTERN.test(path)) {
    const anchor = resolve(CLAUDE_CODE_SURFACE, document);
...
async function trackConversation(): Promise<void> {
  ...
  while (Date.now() < deadline && state && !state.stopped) {
    const path = location.pathname;
    // Claude conversations (/chat/<uuid>) and Claude Code tasks (/code/<id>) — the
    // safe externalSessionRef is the conversation/task URL path only. The
    // /code ROOT itself (the new-task composer) is NOT a task; require a
    // deeper segment so the ref is the created task.
    if (/^\/chat\/.+/.test(path) || /^\/code\/.+/.test(path)) {
      state.conversationPath = path;
      await emitObservation({
        kind: 'progress',
        externalSessionRef: path,
        output: `Claude ${/^\/code\//.test(path) ? 'task' : 'conversation'} ${path}.`,
      });
      return;
    }
...
async function onTranscriptMutation(): Promise<void> {
  ...
  if (hasConfirmationPrompt()) {
    await emitObservation({
      kind: 'blocked',
      reason: 'Claude requires user confirmation.',
  ...
  if (errorText) {
    if (SESSION_EXPIRY_PATTERN.test(errorText)) {
      await emitObservation({
        kind: 'blocked',
        reason: 'Claude session unavailable; please sign in again.',
...
export function currentPhase(): ClaudePhase | null {
  return state?.phase ?? null;
}

/** Aggregate export consumed by the bridge content script. */
export const claudePageRuntime = {
  appliesTo,
  attach,
  stop,
  bindTransport,
