...
/**
 * WORK-031 §26/§31 — Claude page runtime tests against the OBSERVED fixture
...
import {
  ...
  detectSurface,
  appliesTo,
  type ClaudePageSession,
} from '../../src/providers/claude/claude-page-runtime.js';

const PROMPT =
  '# Implementation Instructions — WORK-CLAUDE-001\n\n## Objective\nProve the ChatGPT adapter.';

function makeSession(overrides: Partial<ClaudePageSession> = {}): ClaudePageSession {
  return {
    executionId: 'wf_claude0001',
    workItemLabel: 'WORK-CLAUDE-001',
    provider: 'claude',
...
describe('Claude page runtime (fixture = observed DOM)', () => {
  let sent: Array<{ type: string; executionId: string | null; payload: Record<string, unknown> }>;

  beforeEach(() => {
    sent = [];
    (window as unknown as { __claudeSubmits?: number }).__claudeSubmits = 0;
  ...
  const countSubmits = () => (window as unknown as { __claudeSubmits?: number }).__claudeSubmits ?? 0;

  it('appliesTo(chatgpt) only', () => {
    expect(appliesTo('claude')).toBe(true);
  ...
  it('happy path (CODEX surface): digest verified → injected → submitted ONCE → task → completed', async () => {
    const doc = loadFixture({}, 'claudeCode');
    ...
    window.history.pushState({}, '', '/code/t/claude-abc123');
    await wait(600);
    expect(
      sent.some(
        (s) =>
          s.type === 'EXECUTION_PROGRESS' &&
          s.payload.externalSessionRef === '/code/t/claude-abc123',
      ),
    ).toBe(true);
  ...
  it('implementation on the CHAT surface → BLOCKED "Claude coding environment unavailable or unverified." + ZERO submits (no silent fallback)', async () => {
  ...
  it('detectSurface: /code → coding-agent (HIGH); /chat/<uuid> or root → conversational-chat; /projects → projects; else unknown', () => {
    loadFixture({}, 'claudeCode');
    ...
    window.history.pushState({}, '', '/some-other-surface');
  ...
  it('conversational Chat support is KEPT for conversational tasks (no coding gate)', async () => {
    ...
    expect(countSubmits()).toBe(1);
    expect(
      sent.some((s) => s.type === 'EXECUTION_PROGRESS' && /submitted to Claude/.test(String(s.payload.output))),
    ).toBe(true);
  });
  ...
  it('login wall (buttons) → BLOCKED "Please sign in to Claude." (no submit attempts)', async () => {
    loadFixture({ wall: 'login' });
    const result = await attach(makeSession());
    expect(result.attached).toBe(false);
    const blocked = sent.find((s) => s.type === 'EXECUTION_BLOCKED');
    expect(blocked!.payload.reason).toBe('Please sign in to Claude.');
    expect(countSubmits()).toBe(0);
  });

  it('login wall (/login path) → BLOCKED even without login buttons', async () => {
    const doc = loadFixture({ wall: 'login' });
    void doc;
  ...
  it('session-expiry error text → BLOCKED "Claude session unavailable; please sign in again."', async () => {
    const doc = loadFixture({}, 'claudeCode');
    installFixtureComposer(doc);
    await attach(makeSession());
    ...
    await wait(1600);
    const blocked = sent.find((s) => s.type === 'EXECUTION_BLOCKED');
    expect(blocked!.payload.reason).toBe('Claude session unavailable; please sign in again.');
  });

  it('confirmation prompt → BLOCKED "Claude requires user confirmation." (§16 stop + ask)', async () => {
    const doc = loadFixture({}, 'claudeCode');
    installFixtureComposer(doc);
    await attach(makeSession());
    ...
    await wait(1600);
    const blocked = sent.find((s) => s.type === 'EXECUTION_BLOCKED');
    expect(blocked!.payload.reason).toBe('Claude requires user confirmation.');
  });
  ...
  it('cross-conversation scope: observations stop when the user navigates away (§18)', async () => {
    ...
    installFixtureComposer(doc);
    await attach(makeSession());
    window.history.pushState({}, '', '/code/t/claude-scope-1');
    await wait(600);
    window.history.pushState({}, '', '/code/t/OTHER-task');
