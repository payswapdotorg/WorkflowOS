/**
 * WORK-030 (PR #33 review): Codex fixture agent — deterministic stand-in for
 * the Codex coding-environment task flow on the fixture DOM (codex.html).
 *
 * Real-submit only: the send button enables on input; a submit counts ONLY
 * via a send click on a non-empty composer
 * (window.__codexFixture.submits — the E2E asserts exactly 1).
 *
 * After a submit the fixture creates a task (/codex/t/<id> pushState), shows
...
(function () {
  ...
  if (params.get('wall') === 'login') {
    ...
    if (window.location.pathname.indexOf('/auth/login') !== 0) {
      window.history.replaceState({}, '', '/codex/auth/login');
    }
    return;
  }
